...
import { Activity, Bot, CheckCircle2, Download, GitBranch, Network, Save, ShieldCheck, X } from 'lucide-react';
import type { ReviewActionRecord, SafetySignal, SemanticRuntimeView, StudyEvidence } from '@/lib/contracts';
...
type Canvas = 'evidence' | 'dose' | 'semantics';

export default function InvestigationRoom({ evidence, signal, runtime, onClose }: { evidence: StudyEvidence; signal: SafetySignal; runtime: SemanticRuntimeView; onClose: () => void }) {
  ...
  return <div className="investigation-room" role="dialog" aria-modal="true" aria-label="AI safety investigation room">
    ...
    <div className="room-body">
      ...
      <main className="room-stage">
        <nav className="room-tabs"><button className={canvas === 'evidence' ? 'active' : ''} onClick={() => setCanvas('evidence')}><GitBranch size={14} /> Evidence network</button><button className={canvas === 'dose' ? 'active' : ''} onClick={() => setCanvas('dose')}><Activity size={14} /> Dose & lab response</button><button className={canvas === 'semantics' ? 'active' : ''} onClick={() => setCanvas('semantics')}><Network size={14} /> Agent retrieval plan</button></nav>
