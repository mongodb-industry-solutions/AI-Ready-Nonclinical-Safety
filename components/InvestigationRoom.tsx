    ...
    <div className={`room-body room-body-feed ${preview ? 'inspector-open' : ''} ${documentPreviewOpen ? 'inspector-document' : ''}`}>
      ...
      <main className="room-stage">
        ...
        <section className="room-widget">
          {activeCanvas === 'assistant' && <AgentPanel study={evidence.study} signal={signal} profileId={runtime.activeProfile.id} evidence={evidence} runtime={runtime} expanded onShowSource={() => showSource('MI')} onOpenCoherence={() => navigateCanvas('workspace')} onOpenLiterature={() => navigateCanvas('literature')} onOpenPortfolio={onOpenPortfolio} onOpenSemantic={onOpenSemantic} onInspectLiterature={(document, execution) => setPreview({ title: document.title, detail: `${document.journal} · ${document.year} · PMID ${document.pmid}`, document, execution })} onInspectWidget={(widget) => setPreview(widget === 'dose-response' ? { title: 'Dose-response evidence', detail: 'Microscopic finding incidence across administered-dose groups · MI + DM + TX', widget } : { title: lab?.label || 'Laboratory trajectory', detail: 'Longitudinal laboratory context by treatment group · LB + DM + TX', widget })} />}
