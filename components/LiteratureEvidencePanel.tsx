...
export default function LiteratureEvidencePanel({ signal, documents, profileId, onOpenDocument }: { signal: SafetySignal; documents: LiteratureDocument[]; profileId: SemanticProfileId; onOpenDocument?: (document: RankedLiteratureDocument, execution: LiteratureQueryExecution | null) => void }) {
  ...
  return <div className="literature-workspace">
    ...
    <div className={`literature-grid ${onOpenDocument ? 'literature-list-only' : ''}`}>
      <div className="publication-list">{ranked.map((document) => <button key={document.id} className={document.id === selected.id ? 'active' : ''} onClick={() => { setSelectedId(document.id); onOpenDocument?.(document, execution); }}><span className="publication-rank">{document.retrieval.score}<small>match</small></span><span><em>{roleLabels[document.evidenceRole]}</em><b>{document.title}</b><small>{document.journal} · {document.year} · PMID {document.pmid}</small><span className="retrieval-lanes">{document.retrieval.lanes.join(' · ')}</span></span></button>)}</div>
    ...
    <footer className="document-provenance"><FileText size={13} /><span><b>Document architecture</b>PubMed metadata/abstracts via API · licensed PDFs in S3-compatible storage · parsed chunks, Atlas-managed embeddings, links, and citations</span><em>{execution ? `${execution.semanticReleaseId} · ${execution.profileId}` : 'No full text redistributed in this fixture'}</em></footer>
  </div>;
}
