...
const apiContracts = [
  ...
  { method: 'POST', path: '/api/investigations', title: 'AI investigation', input: 'studyId, signalId, profileId, sessionId, question', output: 'Cited EvidenceBrief + typed widget receipts + executed plan', policy: 'Scope-bound memory; deterministic evidence; read-only tools' },
    ...
    <section className="architecture-plane runtime-plane">
      ...
      <div className="runtime-map">
        ...
        <div className="atlas-core">
          ...
          <div className="atlas-planes"><div><span>Immutable evidence</span><code>study_snapshots</code><code>study_snapshot_pointers</code><code>dataset_definitions · cdisc_records</code><code>subjects · source_artifacts</code></div><div><span>Reconciled projections</span><code>study_evidence · endpoint_summaries</code><code>measurement_series · subject_timelines</code><code>portfolio_findings · autoEmbed</code><code>literature_chunks · evidence_relationships</code></div><div><span>Semantic release</span><code>semantic_releases.bundle</code><code>semantic_resources · semantic_edges</code><code>semantic_search_documents · autoEmbed</code></div><div><span>Solution state</span><code>investigation_sessions · investigations</code><code>agent_checkpoints · checkpoint_writes</code><code>review_actions · semantic_change_events</code></div></div>
        ...
        <div className="runtime-consumers"><span className="map-column-label">Interfaces & experience</span><ArchitectureNode icon={Code2} tone="cyan" eyebrow="Stable boundary" title="Solution-owned APIs" detail="Contracts, profile policy, execution telemetry and audit" tags={['REST + SSE']} compact /><ArchitectureNode icon={Bot} mdb="ai" tone="violet" eyebrow="Bundled Magenta" title="Memory + orchestration" detail="Interpret intent, invoke governed resolvers, and select typed visuals" tags={['profile-bound']} compact /><ArchitectureNode icon={Workflow} tone="green" eyebrow="Next.js solution" title="Safety Investigation Room" detail="Deterministically hydrated widgets, hypotheses and expert decisions" tags={['human in control']} compact /></div>
      </div>
    </section>
...
function DataModel({ evidence, runtime }: { evidence: StudyEvidence; runtime: SemanticRuntimeView }) {
  ...
  provenance: {
    ...
    projectionDigest, reconciliation, projectionRuleIds
      ...
      <article className="projection-stack"><header><span className="panel-kicker">Independent, rebuildable projections</span><h2>Enrichment without evidence mutation</h2></header><div><Search size={15} /><span><b>Search documents</b><small>Evidence and semantic text, facets and source references</small></span><code>evidence_chunks · semantic_search_documents</code></div><div><Sparkles size={15} /><span><b>Automated embeddings</b><small>Atlas embeds indexed text and query text without vector fields in source documents</small></span><code>__mdb_internal_search</code></div><div><GitBranch size={15} /><span><b>Materialized relationships</b><small>Definition edges and Finding → publication → passage evidence paths remain distinct</small></span><code>semantic_edges · evidence_relationships</code></div><div><ShieldCheck size={15} /><span><b>Expert workflow</b><small>Hypotheses and decisions remain separate from observations</small></span><code>investigations · review_actions</code></div></article>
    </div>
    <div className="model-rules"><article><b>Fidelity</b><p>Every derived fact carries study, snapshot, domain and source lineage back to the CDISC artifact.</p></article><article><b>Workload fit</b><p>The application does not repeatedly reconstruct one visual question from many tabular domain joins.</p></article><article><b>Evolution</b><p>New retrieval text, relationships or terminology mappings are additive projections; Atlas maintains embeddings separately.</p></article><article><b>Portability</b><p>Archetypes and storage bindings keep semantic meaning independent from one physical representation.</p></article></div>
  </div>;
}
...
function Interfaces({ selectedApi, onSelectApi }: { selectedApi: number; onSelectApi: (index: number) => void }) {
  ...
  const queryStages = [['01', 'Understand intent', 'session memory · current question'], ['02', 'Authorize & scope', 'profile · study · immutable snapshot'], ['03', 'Resolve evidence', 'exact · lexical · vector · graph'], ['04', 'Fuse & rerank', 'RRF + domain-aware relevance'], ['05', 'Select + hydrate', 'Magenta receipt · deterministic widget data'], ['06', 'Synthesize + review', 'cited hypothesis · expert action']];
...
function ArchitectureValue() {
  ...
  return <div className="architecture-value-view">
    ...
    <section className="truth-model"><header><span className="panel-kicker">A deliberate separation of truth</span><h2>One evidence chain, different responsibilities</h2></header><div><article><span>Observed evidence</span><b>CDISC-derived snapshots</b><p>Immutable facts that answer what was observed in the study.</p></article><article><span>Derived retrieval</span><b>Chunks, Atlas indexes and edges</b><p>Rebuildable projections that answer where relevant context may exist.</p></article><article><span>Semantic contract</span><b>Compiled release + active pointer</b><p>Versioned meaning that governs concepts, archetypes, profiles and resolvers.</p></article><article><span>Expert state</span><b>Investigations and decisions</b><p>Append-only workflow that records what humans concluded or requested next.</p></article></div></section>
